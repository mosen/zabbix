...
// ZFE Notes:
// Differences between this and the original implementation are just the ability to attach to a contentEditable
// div which is not necessarily an INPUT or behaves like an INPUT.

var ZFE_LCL_SUGGESTS = [];

function createZFESuggest(oid) {
	var sid = ZFE_LCL_SUGGESTS.length;
	ZFE_LCL_SUGGESTS[sid] = new ZFESuggest(sid, oid);

	return ZFE_LCL_SUGGESTS[sid];
}
...
/**
 * ZFE Autocomplete / Suggest Widget.
 *
 * This Widget differs from CSuggest in the following ways:
 *
 * - The input element (and its event listeners) are controlled externally to this widget.
 * - The method `suggest` must be called when the input is in a state to suggest something.
 * - We do not use the caching mechanism from CSuggest (for now)
 */
var ZFESuggest = Class.create({
	// public
	'useLocal':			true,	// use cache to find suggests
	'useServer':		false,	// use server to find suggests
	...
	'dom': {
		'relativeEl':  null, // DOM node which the suggest div should be placed relative to.
	...
	initialize: function(id, relativeEl, suggestEl) {
		this.id = id;
		this.dom.relativeEl = $(relativeEl);
		this.dom.suggest = $(suggestEl);
	...
	/**
	 * Suggest auto completion values.
	 *
	 * @param {jQuery} relativeTo The element where the suggestion list should be constructed.
	 * @param {String} value 			The keyword(s) to use for suggestion.
	 */
	suggest: function(relativeTo, value) {
		this.hlIndex = 0;
		this.suggestCount = 0;
	...
	searchCache: function(needle) {
		this.needles[needle].list = {'a': 'agent.ping: Agent availability check'};
		return true;
	},
	// cache
	// searchCache: function(needle) {
	...
	positionSuggests: function() {
		...
		var pos = jQuery(this.dom.relativeEl).offset(),
			dims = getDimensions(this.dom.relativeEl);

		var caretPos = getCaretPosition(this.dom.relativeEl);
		console.log('caret position');
		console.log(caretPos);

		this.dom.suggest.style.top = (pos.top + caretPos[1] * 10) + 'px';
		this.dom.suggest.style.left = (pos.left + caretPos[0] * 5) + 'px';
	},
